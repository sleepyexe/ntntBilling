...
import { useSocietyData } from "@/state/society";
type Props = {};

const SocietyInvoices = (props: Props) => {
  const [pages, setPages] = usePages();
  const [back, setBack] = useBack();
  const setContainer = useSetModalContainer();
  const data: SocietyProps[] = useSocietyData();
  ...
  return (
    ...
    >
      <button
        ...
        className="w-fit bg-none absolute self-end pr-1 pt-1"
      >
        <XIcon />
      </button>
...
export default SocietyInvoices;
